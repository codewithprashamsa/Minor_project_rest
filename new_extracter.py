...
protocol / TLS (JA3, JA4 & JA4S) / DNS / HTTP features per flow, joins each
...
# Numeric representation used for the ja4_tls_ver_num / ja4s_tls_ver_num columns.
...
# --------------------------------------------------------------------------- #
# TLS ServerHello manual parsing (used for JA4S). Structurally similar to the
# ClientHello but with a single chosen cipher instead of a cipher list, and no
# SNI. Never raises.
# --------------------------------------------------------------------------- #

@dataclass
class TLSServerHelloInfo:
    server_version: int
    cipher: int
    extensions: List[int] = field(default_factory=list)
    alpn: List[str] = field(default_factory=list)
    supported_version: Optional[int] = None  # from the supported_versions extension (TLS 1.3 actual version)


def parse_tls_server_hello(payload: bytes) -> Optional[TLSServerHelloInfo]:
    """
    Best-effort manual parser for a TLS ServerHello embedded in a TCP payload.
    Returns None if the payload doesn't look like a (plaintext) ServerHello,
    or if parsing fails for any reason. Never raises.
    """
    try:
        if len(payload) < 9:
        ...
        # record header: type(1) version(2) length(2)
        pos = 5
        if payload[pos] != TLS_SERVER_HELLO_TYPE:
            return None
        # handshake header: type(1) length(3)
        pos += 4
        ...
        if len(payload) < pos + 1:
            return None
        pos += 1  # compression method (single byte in ServerHello)
        ...
        if pos + 2 <= len(payload):
            ...
            while pos + 4 <= ext_end:
                ...
                try:
                    if ext_type == EXT_ALPN and len(ext_data) >= 2:
                        sub_pos = 2  # skip protocol_name_list length
                        while sub_pos + 1 <= len(ext_data):
                            proto_len = ext_data[sub_pos]
                            proto = ext_data[sub_pos + 1:sub_pos + 1 + proto_len]
                            if proto:
                                alpn.append(proto.decode("utf-8", errors="ignore"))
                            sub_pos += 1 + proto_len
                    elif ext_type == EXT_SUPPORTED_VERSIONS and len(ext_data) >= 2:
                        # In ServerHello this extension holds a single 2-byte
                        # version (no list-length prefix like in ClientHello).
...
def compute_ja4s(info: TLSServerHelloInfo, transport: str = "t") -> Dict[str, object]:
    """
    Best-effort implementation of the JA4S fingerprint (FoxIO) for a TCP TLS
    ServerHello. JA4S mirrors JA4's construction but is built from the
    server's single chosen cipher and the (unsorted, in-wire-order)
    extensions list, since a ServerHello has no cipher list and no SNI.

    Construction: JA4S_a + "_" + JA4S_b + "_" + JA4S_c
      JA4S_a: transport + version + extension_count(2 digits) + alpn(2 chars)
      JA4S_b: the single negotiated cipher, as 4 lowercase hex digits
      JA4S_c: sha256 of extensions in wire order (not sorted), truncated to 12 chars
    """
    exts_ng = [e for e in info.extensions if e not in GREASE_VALUES]

    # Version: TLS 1.3 negotiates via the supported_versions extension in the
    # ServerHello; fall back to the legacy record-level version otherwise.
    version_val = info.supported_version if info.supported_version is not None else info.server_version
    ver_token = JA4_VERSION_TOKEN.get(version_val, "00")
    ver_num = JA4_VERSION_NUM.get(ver_token, 0)
    ...
    ja4s_a = "{proto}{ver}{ec:02d}{alpn}".format(
        proto=transport, ver=ver_token, ec=ext_count, alpn=alpn_part,
    )

    ja4s_b = f"{info.cipher:04x}"

    if exts_ng:
        ext_str = ",".join(f"{e:04x}" for e in exts_ng)  # wire order, not sorted
        ja4s_c = hashlib.sha256(ext_str.encode()).hexdigest()[:12]
    else:
        ja4s_c = "0" * 12

    fingerprint = f"{ja4s_a}_{ja4s_b}_{ja4s_c}"

    return {
        "ja4s_fingerprint": fingerprint,
        "ja4s_tls_ver_num": ver_num,
        "ja4s_cipher": ja4s_b,
        "ja4s_ext_count": len(exts_ng),
    }
...
class FlowStats:
    ...
    def __init__(self, key: FlowKey):
        ...
        self.tls_server_info: Optional[TLSServerHelloInfo] = None  # first ServerHello seen
    ...
    def maybe_set_tls_server(self, info: TLSServerHelloInfo) -> None:
        if self.tls_server_info is None:
            self.tls_server_info = info
    ...
    def finalize(self, handshake_tracker: HandshakeTracker) -> Dict[str, object]:
        ...
        if self.tls_server_info is not None:
            ja4s_fields = compute_ja4s(self.tls_server_info)
            row.update(ja4s_fields)
            # A ServerHello was seen on this directional flow, so it carries
            # TLS content even if no ClientHello happened to be captured here.
            row["has_tls"] = 1
        else:
            row.update({
                "ja4s_fingerprint": "",
...
class PcapProcessor:
    ...
    def _process_packet(self, pkt, flows: Dict[FlowKey, FlowStats], handshake_tracker: HandshakeTracker) -> None:
        ...
        if protocol == "TCP" and payload_bytes[:1] == b"\x16":
            client_info = parse_tls_client_hello(payload_bytes)
            if client_info is not None:
                flow.maybe_set_tls(client_info)
            else:
                server_info = parse_tls_server_hello(payload_bytes)
                if server_info is not None:
                    flow.maybe_set_tls_server(server_info)
...
def print_summary(rows: List[Dict[str, object]]) -> None:
    ...
    ja4s_values = [r["ja4s_fingerprint"] for r in rows if r.get("ja4s_fingerprint")]
    unique_ja4s = len(set(ja4s_values))
    top_ja4s = Counter(ja4s_values).most_common(20)
    ...
    if top_ja4:
        ...
        print("  (no TLS ClientHello flows found)")

    print(f"\nUnique JA4S fingerprints: {unique_ja4s}")
    print("Top 20 most common JA4S fingerprints:")
    if top_ja4s:
        for fp, count in top_ja4s:
            print(f"  {count:6d}  {fp}")
    else:
        print("  (no TLS ServerHello flows found)")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Build a flow-level JA4/JA4S/JA3 malware traffic dataset from a directory of PCAPs."
    )
